...
async function consolidateUser(userId: string): Promise<void> {
  try {
    ...
    const instructions = [
      ...
      `Usunięto ${consolidateReport.deleted} technicznych duplikatów. Pozostało ${consolidateReport.remaining} wpisów.`,
      `Wykonaj kolejno:`,
      `1. memory_query(kind='facts', userId='${userId}') — pobierz wszystkie fakty.`,
      `2. Usuń semantyczne duplikaty (memory_delete_fact), popraw nieprecyzyjne (memory_upsert_fact).`,
      `3. memory_record_event(type='knowledge_consolidation', payload={deleted_programmatic: ${consolidateReport.deleted}, reviewed: true}).`,
      `4. Wyślij do użytkownika raport (plain tekst bez Markdown):`,
      `[Nocna reorganizacja ${today}] Usunieto duplikatow: ${consolidateReport.deleted}. Przejrzano semantycznie: X. Wpisow w bazie: Y. Mapa wiedzy: [dla kazdego tematu 1 linijka z kluczowymi faktami]. Podsumowanie: [1 zdanie].`,
    ].join('\n');
